...
pub fn render(f: &mut Frame, app: &mut App) {
    ...
    let installed_groups = app
        .groups
        .iter()
        .filter(|g| !g.packages.is_empty() && g.packages.iter().all(|p| p.1))
        .count();
    let partial_groups = app
        .groups
        .iter()
        .filter(|g| {
    ...
    let stats_line = Line::from(vec![
        Span::styled(
            format!(" groups installed : {} ", installed_groups + partial_groups),
    ...
    if app.filtered_groups.is_empty() {
        ...
        let items: Vec<ListItem> = app
            ...
            .map(|group| {
                ...
                let status_color = if installed == total && total > 0 {
                    Color::Green
                } else if installed > 0 {
                    Color::Yellow
                } else {
                ...
                ListItem::new(Line::from(vec![
                    Span::styled(
                        fav_icon,
                        Style::default().fg(if group.is_favorite {
                            Color::Yellow
                        } else {
                            Color::Cyan
                        }),
        ...
        let list = List::new(items)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(format!(" groups ({}) ", app.filtered_groups.len())),
            )
            .highlight_style(
                Style::default()
                    .bg(Color::Cyan)
