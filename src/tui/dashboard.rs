...
pub fn render(f: &mut Frame, app: &mut App) {
    ...
    let padding = if area.height > total_fixed {
        (area.height - total_fixed) / 2
    } else {
        0
    };
    ...
    if show_shark && chunks[2].height > 0 {
        ...
        let v_pad = if chunk_height > art_height {
            (chunk_height - art_height) / 2
        } else {
            0
        };
        ...
        let blahaj_box = Paragraph::new(app.dashboard_art.clone()).alignment(Alignment::Center);
    ...
    let news_color = if has_critical {
        Color::LightRed
    ...
    let stats_text = Line::from(vec![
        ...
        Span::styled("   ·   ", Style::default().fg(Color::DarkGray)),
        Span::styled(
            format!("󰆴 {} orphans", app.orphan_count),
